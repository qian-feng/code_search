...
class callgraph_based_matching:
	...
	def __init__(self, version_src, version_dst, mode, dir_path):
		self.indexes = {}
		self.mode = mode
		self.funcbodylist = {}
		self.funccfglist = {}
		...
		self.version = {'src': version_src, 'dst': version_dst}
...
if __name__=="__main__":
	try:
		fp = open("E:/Contexted_based_match/arguments", 'r')
	except:
		fp = open("/media/sf_qian/vmi/Contexted_based_match/arguments",'r')
	args = []
	for x in fp:
		x = x.split('\n')[0]
		args.append(x)
	ea = FirstSeg()
	version = args[1].split("\r")[0]
	mod = args[2].split("\r")[0]
	version_src = args[3].split('\r')[0]
	version_dst = args[4].split('\r')[0]
	home_dir = args[5].split('\r')[0]
	# ***************Experiment II: Bipartite Graph Matching ***************************

	fg = callgraph_based_matching(version_src, version_dst, mod, home_dir)
	
	# 1. matrix generation
	if mod == 'start':
		fg.feature_generating(ea, version)
		exit()
	else:
		fg.feature_generating(ea, version)
	'''
	...
	#pdb.set_trace()
	cost_matrix = fg.matrix_generating(version_src, version_dst)
	path = home_dir + '/data/output/' + version + '.cost_matrix'
	print 'Matrix is generated!'
	print 'current is dumpping....'
	#pdb.set_trace()
	fg.dumpMatrix(cost_matrix, path)
	print 'dumpping is finished!'

	print 'mapping is working....'
	mapping = hungarian.lap(cost_matrix)
	#pdb.set_trace()
	
	print 'Matching is finished!'
	
	#cost_matrix = pickle.load(open("/home/qian/workspace/code_retriever/code_based/vmi/data/output/win_gt_cost_matrix",'r'))
	#mapping = pickle.load(open("/home/qian/workspace/code_retriever/code_based/vmi/data/output/win_gt_mapping",'r'))
	fg.random_evaluation(10, mapping[0], cost_matrix)
	fg.individual_evaluation(mapping[0], cost_matrix)
	fg.PGM_evaluation(mapping[0], cost_matrix)
	
	#fg.dump(cost_matrix, version_src, version_dst)
	
	#****************Experiment I2: inter-verion function classification ********************************
	# 1. utilizing the function code as signature to construct the detector for the function detection in the new version
