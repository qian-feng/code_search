...
###### --- add it by wmh on 2015.3.24 ---
sys.path.append("/home/qian/code_search/ida-6.6/python");
sys.path.append("/home/qian/code_search/ida-6.6/loaders");
sys.path.append('/home/qian/code_search/mcsema-master/build/mc-sema/bin_descend')
###### --- end add it by wmh on 2015.3.24 ---
...
class ToFileStdOut(object):
    def __init__(self):
        self.outfile = open("/home/qian/idaout.txt", "w")
...
def recoverCfg(to_recover, outf, exports_are_apis=False):
    ...
    Fl.write("len: %d" % len(our_entries))
...
#############
    #defsfile = open("/home/tony/project/mcsema/mcsema/mc-sema/std_defs/std_defs.txt", "r");
    defsfile = open("/home/qian/std_defs.txt", "r");
    em_update, emd_update = parseDefsFile(defsfile)
    EMAP.update(em_update)
    EMAP_DATA.update(emd_update)
...
###########
    ...
    extrnl_func_list = get_extrn_funcs()
    Fl.write("len: %d" % len(extrnl_func_list));
    for funcname in extrnl_func_list:
    	if funcname not in EMAP and funcname not in EMAP_DATA:
	    	Fl.write(funcname + " 12 C N");
###########
