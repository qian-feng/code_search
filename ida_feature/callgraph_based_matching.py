...
class callgraph_based_matching:
	...
	def __init__(self, exename, mode, dir_path):
		...
		self.funcseqs_dirpath = self.home_dir

	def feature_generating(self, ea, version):
			if self.mode != 'cache':

				self.funcseqs = func.get_func_sequences(ea)
				self.funcbodylist = func.get_func_names(ea)
				...
				self.dumpObj(self.funcseqs, self.funcseqs_dirpath, version, 'func_seqs')
...
if __name__=="__main__":
	...
	#exename ='test'
	#mod = 'start'
	#home_dir = '/home/qian/code_search/crossarch/coreutil_i386_o0_'
